...
function itemSelected(e) {
    ...
    if (['circle','triangle','ellipse','path','rect','line','polygon'].indexOf(canvas.getActiveObject().get('type'))!=-1) {
        ...
        if(canvas.getActiveObject().get('type')=='rect' || canvas.getActiveObject().get('type')=='circle') {
    ...
    if (['i-text','heading','list','paragraph'].indexOf(canvas.getActiveObject().get('type')) != -1) {
        canvas.getActiveObject().setControlsVisibility({
            bl: false,
            br: false,
            tl: false,
    ...
    if (canvas.getActiveObject().get('type') == 'chart') {
        $('.actionsButtonsChart').show();
    } else {
        $('.actionsButtonsChart').hide();
    }

    if (canvas.getActiveObject().get('type') == 'image') {
        $('.change_image_container').show();
    } else {
        $('.change_image_container').hide();
    }
...
function addFont(droppedObject, event) {    
    ...
    if (event != null) {
        ...
        var x = event.clientX - (offset.left + imageOffsetX);
        let texts;
        if($(droppedObject).data('type')=='heading') {
            new fabric.Heading(text, {
                left: x,
                top: y,
                fontFamily: 'Arial',
                fontSize: $(droppedObject).data('size'),
                disableStyleCopyPaste : false,
                text_type: $(droppedObject).data('type')
            });
        } else {
            new fabric.Paragraph(text, {
                left: x,
                top: y,
                fontFamily: 'Arial',
                fontSize: $(droppedObject).data('size'),
...
function addShape(dropppedObject, event) {
    ...
    switch ($(dropppedObject).data('name')) {
        ...
        case 'line':
            ...
            break;
        case 'triangle2':
            shape =  new fabric.Path($(dropppedObject).data('path'), {
                fill: 'black',
                stroke: 'black',
                strokeWidth: 2,
                originX : 'center',
        ...
        var chart = c3.generate({
            ...
            onrendered : function() {
                setTimeout(() => {
                    ...
                    if(canvas.getActiveObject().get('fontFamily')) {
...
function addImage(imgLink, new_one = false) {
    if (new_one) {
        ...
        canvas.getActiveObject().setSrc(imgLink, function (image) {
...
function privew() {
    setTimeout(function() {
        $('#img01').html(canvas.toSVG())
        $('#img02').html(canvas.toSVG())
    }, 500);
}
    ...
    if([37,38,39,40].indexOf(key)!= -1){
        if(!iskeyActive) {
            ...
            if (key === 37) { // handle Left key
                ...
                if(options.shiftKey) {
                    moveSelected(Direction.LEFT,10);
                    keyHoldInterval = setInterval(function(){ moveSelected(Direction.LEFT,10); }, 100);
                } else {
                    moveSelected(Direction.LEFT);
                    keyHoldInterval = setInterval(function(){ moveSelected(Direction.LEFT); }, 100);
                }
            } else if (key === 38) { // handle Up key
                options.preventDefault();
                if(options.shiftKey) {
                    moveSelected(Direction.UP,10);
                    keyHoldInterval = setInterval(function(){ moveSelected(Direction.UP,10); }, 100);
                } else {
                    moveSelected(Direction.UP);
                    keyHoldInterval = setInterval(function(){ moveSelected(Direction.UP); }, 100);
                }
            } else if (key === 39) { // handle Right key
                options.preventDefault();
                if(options.shiftKey) {
                    moveSelected(Direction.RIGHT,10);
                    keyHoldInterval = setInterval(function(){ moveSelected(Direction.RIGHT,10); }, 100);
                } else {
                    moveSelected(Direction.RIGHT);
                    keyHoldInterval = setInterval(function(){ moveSelected(Direction.RIGHT); }, 100);
                }
            } else if (key === 40) { // handle Down key
                options.preventDefault();
                if(options.shiftKey) {
                    moveSelected(Direction.DOWN,10);
                    keyHoldInterval = setInterval(function(){ moveSelected(Direction.DOWN,10); }, 100);
                } else {
                    moveSelected(Direction.DOWN);
                    keyHoldInterval = setInterval(function(){ moveSelected(Direction.DOWN); }, 100);
                }
            }
        }
    }
...
function moveSelected(direction, step = 1) {
...
$(document).on('click', '#editChart', function() {
    $('#myCenterModalLabel').text(canvas.getActiveObject().get('chartType')+' Chart');
    if(canvas.getActiveObject().get('type') == 'chart') {
        activeChartObject = canvas.getActiveObject();
        let html = '';
        //If pie Chart here
        if(activeChartObject.chartType=="pie") {
            html += `
            <div class="row">
                <div class="col-12">
                    <p>Note : All fields are editable. Simply click on any field to update the values.</p>
                <div>
                <div class="col-12 mb-2">
                    <button class="btn btn-success btn-xs add_row mr-1">Add Row</button>
        ...
        if(activeChartObject.chartType=="bar" || activeChartObject.chartType=="line" || activeChartObject.chartType=="area" || activeChartObject.chartType=="area-spline") {
            html += `<div class="row">
                ...
                <div class="col-12 mb-2">
                    <button class="btn btn-success btn-xs add_row mr-1">Add Row</button>
                    <button class="btn btn-danger btn-xs delete_row">Delete Row</button> 
                    <button class="btn btn-success btn-xs add_column mr-1">Add Column</button>
                    <button class="btn btn-success btn-xs delete_column mr-1">Delete Column</button>
                </div>
            `;
            let i = 0;
            if(activeChartObject.chartData.colors) {
    ...
    $('#ChartFontFamily').val(canvas.getActiveObject().get('fontFamily')).selectpicker('refresh');
});
...
$(document).on('click', '.update_chart', function() {
    ...
    if(activeChartObject.chartType=="pie" || activeChartObject.chartType=="donut") {
        ...
        canvas.getActiveObject().set('textColor',$('.makeItColorPicker').data('value'));
        canvas.getActiveObject().set('fontFamily',$('#ChartFontFamily').val());
        if(activeChartObject.chartType=="donut") {
            canvas.getActiveObject().set('chartTitle',$('.chartTitle').val());
        }
        canvas.getActiveObject().set('chartData',activeChartObject.chartData);
        canvas.getActiveObject().set('showLabel',show_label);
        updateChart();
    }
...
function openNav() {
    $('.overlay-content').html(canvas.toSVG());
